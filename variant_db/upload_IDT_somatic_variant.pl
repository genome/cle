...
for my $cwl_dir (@cwl_dirs) {
    ...
    for my $case_dir (@case_dirs) {
        ...
        while (my $line = $fh->getline) {
            ...
            my @info = ($assay_version, $pipeline_version, $date, $case_name, @columns);
            my $sth = $dbh->prepare("insert into $tablename (assay_version, pipeline_version, date, sample, chromosome, position, rs_id, reference, variant, variant_callers, normal_gt, normal_ad, normal_af, normal_dp, tumor_gt, tumor_ad, tumor_af, tumor_dp, consequence, symbol, feature_type, feature, HGVSc, HGVSp, cDNA_position, cds_position, protein_position, amino_acids, codons, HGNC_ID, existing_variation, gnomADe_af) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)") or die $DBI::errstr;
