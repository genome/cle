...
for my $case (readdir $dir_h) {
    ...
    my $err_log  = File::Spec->join($log_dir, $case.'_xfer.err');
    my $out_log  = File::Spec->join($log_dir, $case.'_xfer.out');
    my $dest_dir = $case.'/';

    my $cmd = "bsub -e $err_log -o $out_log -q $queue -a 'docker($image)' scp -i $private_key -r $local_tmp_dir $host:$dest_dir";
    system $cmd;
    #print $cmd."\n\n";
}
